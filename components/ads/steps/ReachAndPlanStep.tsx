...
export function ReachAndPlanStep({ state, errors, patch }: { state: AdWizardData; errors: AdFieldErrors; patch: PatchAdWizard }) {
  const durations = { BRONZE: 1, SILVER: 3, GOLD: 6 } as const;
  return (
    <Card className="space-y-6 border border-slate-200 p-6 shadow-xs">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {AD_PLANS.map((plan) => {
          ...
          return <button key={plan} type="button" onClick={() => patch({ plan, durationMonths: durations[plan] })} className={cn('relative rounded-2xl border-2 p-6 text-left transition hover:border-[#078cf5]', selected ? plan === 'SILVER' ? 'border-[#173247] bg-[#fff98a] shadow-lg' : 'border-[#078cf5] bg-white shadow-sm' : 'border-slate-200 bg-white')}>
            ...
            <p className="mt-2 text-[28px] font-extrabold text-[#078cf5]">{formatAdCurrency(item.monthlyPriceCents * durations[plan])}</p>
            <ul className="mt-4 space-y-2 text-[11px] text-[#324b5f]"><li className="flex gap-2"><Check size={13} className="text-[#078cf5]" />{durations[plan]} {durations[plan] === 1 ? 'mes' : 'meses'}</li>{item.highlights.map((highlight) => <li key={highlight} className="flex gap-2"><Check size={13} className="text-[#078cf5]" />{highlight}</li>)}</ul>
